...
class MaskROIOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        if is_train:
            ...
            pred_boxes = bbox_pred(rois.asnumpy()[:, 1:], bbox_deltas.asnumpy(), (0.1, 0.1, 0.2, 0.2))
        else:
            label = np.argmax(in_data[3].asnumpy()[0], axis=1)
            pred_boxes = bbox_pred(rois.asnumpy()[:, 1:], bbox_deltas.asnumpy()[0], (0.1, 0.1, 0.2, 0.2))
